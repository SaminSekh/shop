class AuthManager {
    ...
    async login(username, password) {
        ...
        try {
            ...
            // Block Login Check: If shop is paused, prevent non-super_admin login
            if (user.role !== 'super_admin' && user.shop_id) {
                const { data: shop } = await supabaseClient
                    .from('shops')
                    .select('is_paused, shop_name')
                    .eq('id', user.shop_id)
                    .single();

                if (shop && shop.is_paused) {
                    throw new Error(`Access Denied: Your shop (${shop.shop_name}) is currently suspended. Please contact the Super Admin.`);
                }
            }
