...
class InventoryManager {
            ...
            if (this.products.length > 0) {
                ...
                if (variantStocks && variantStocks.length > 0) {
                    ...
                    variantStocks.forEach(v => {
                        ...
                        const vCost  = parseFloat(v.cost_price) || 0;
                        const vSell  = parseFloat(v.price) || 0;
                        stockTotals[v.product_id] = (stockTotals[v.product_id] || 0) + vStock;
                        costTotals[v.product_id]  = (costTotals[v.product_id]  || 0) + (vStock * vCost);
                        sellTotals[v.product_id]  = (sellTotals[v.product_id]  || 0) + (vStock * vSell);
                        // Track price range across variants
                        if (vCost > 0) {
                    ...
                    this.products.forEach(p => {
                        if (stockTotals[p.id]) {
                            ...
                            p.stock = p._baseStock + stockTotals[p.id];
                        }
                        ...
                        if (minCost[p.id] !== undefined) {
                            p._variantMinCost = minCost[p.id];
                            p._variantMaxCost = maxCost[p.id];
                        }
                        if (minSell[p.id] !== undefined) {
                            p._variantMinSell = minSell[p.id];
                            p._variantMaxSell = maxSell[p.id];
                        }
                    });
                }
            }
    ...
    renderProducts() {
        ...
        const productRows = pageProducts.map(product => {
            const hasVariants = product._variantMinSell !== undefined;

            // Cost price display
            let costDisplay, sellDisplay, marginDisplay;
            if (hasVariants) {
                const baseCp = parseFloat(product.cost_price) || 0;
                const baseSp = parseFloat(product.selling_price) || 0;
                // Include base product price in the range
                const minC = Math.min(baseCp, product._variantMinCost ?? baseCp);
                const maxC = Math.max(baseCp, product._variantMaxCost ?? baseCp);
                const minS = Math.min(baseSp, product._variantMinSell ?? baseSp);
                const maxS = Math.max(baseSp, product._variantMaxSell ?? baseSp);
                ...
                // Weighted avg margin: include base stock value too
                const baseStock = parseInt(product._baseStock !== undefined ? product._baseStock : 0) || 0;
                const totalCostVal = (product._variantCostValue || 0) + (baseStock * baseCp);
                const totalSellVal = (product._variantSellValue || 0) + (baseStock * baseSp);
            ...
            const profitMarginNum = hasVariants
                ? (product._variantCostValue > 0
                    ? ((product._variantSellValue - product._variantCostValue) / product._variantCostValue * 100)
                    : 0)
