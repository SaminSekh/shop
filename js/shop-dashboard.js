...
class ShopDashboardManager {
    ...
    async init() {
        ...
        // Load shop data FIRST (needed for logo fallbacks)
    ...
    async updateShopName() {
        try {
            const { data: shop, error } = await supabaseClient
                .from('shops')
                .select('shop_name, shop_logo, current_balance')
                .eq('id', this.shopId)
                .single();
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                ...
                                if (dateLabel === 'Today') {
                                    return `Today's Sales: ₹${value.toFixed(2)}`;
                                }
                                return `${dateLabel}: ₹${value.toFixed(2)}`;
                            },
