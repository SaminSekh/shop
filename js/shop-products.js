...
class ShopProductsViewer {
    ...
    async openProductDetail(productId) {
        ...
        if (variants.length > 0) {
            ...
            if (thumbs) {
                // Collect primary product images from all possible sources
                let primaryImages = [];
                if (product.product_image) primaryImages.push(product.product_image);
                // From product_images column
                ...
                if (Array.isArray(extraImages)) {
                    extraImages.forEach(url => { if (url && !primaryImages.includes(url)) primaryImages.push(url); });
                }
                // Also check metadata.product_images as fallback
                let meta = product.metadata || {};
                if (typeof meta === 'string') { try { meta = JSON.parse(meta); } catch(e) { meta = {}; } }
                const metaImages = meta.product_images || [];
                if (Array.isArray(metaImages)) {
                    metaImages.forEach(url => { if (url && !primaryImages.includes(url)) primaryImages.push(url); });
                }

                // Build a map: image URL → variant (for variant images only)
                const imageToVariantMap = {};
                variants.forEach(v => {
                    // Primary variant image
                    if (v.image_url && !primaryImages.includes(v.image_url)) {
                        imageToVariantMap[v.image_url] = v;
                    }
                    // Additional variant images from attributes.variant_images
                    ...
                    variantImgs.forEach(url => {
                        if (url && !primaryImages.includes(url)) {
                            imageToVariantMap[url] = v;
                        }
                    });
                });

                // Build full image list: primary images first, then variant images
                let imageList = [...primaryImages];
                variants.forEach(v => {
                    if (v.image_url && !imageList.includes(v.image_url)) imageList.push(v.image_url);
                    let vAttrs = v.attributes || {};
                    if (typeof vAttrs === 'string') { try { vAttrs = JSON.parse(vAttrs); } catch(e) { vAttrs = {}; } }
                    const variantImgs = vAttrs.variant_images || [];
                    variantImgs.forEach(url => { if (url && !imageList.includes(url)) imageList.push(url); });
                });

                // Safety: ensure the main product image is always in the list
                const mainImgUrl = this.getAssetUrl(product.product_image || '');
                if (mainImgUrl && !imageList.includes(mainImgUrl) && !mainImgUrl.includes('default-product')) {
                    imageList.unshift(mainImgUrl);
                    primaryImages.unshift(mainImgUrl);
                }
                    ...
                    imageList.forEach((url, idx) => {
                        ...
                        t.onclick = () => {
                            ...
                            if (matchedVariant) {
                                ...
                                addBtn.onclick = (e) => { self.addToCart(product.id, e); };
                                // Highlight base button, deselect others
                                document.querySelectorAll('.variant-select-btn').forEach(btn => {
                                    if (btn.dataset.idx === 'base') {
                                ...
                                if (specsBox) {
                                    ...
                                    if (baseMeta && typeof baseMeta === 'object') {
                                        ...
                                        for (const [k, val] of Object.entries(baseMeta)) {
                                            const vs = String(val);
                                            if (val && !skip.includes(k) && !k.toLowerCase().includes('image') && !Array.isArray(val) && typeof val !== 'object' && !vs.includes('://') && !vs.startsWith('[') && !vs.startsWith('data:')) {
            ...
            function selectVariant(v) {
                ...
                if (specsBox) {
                    ...
                    const skip = ['name', 'product_images', 'product_image', 'variant_images', 'variant_group', 'variant_size', 'variant_color', 'variant_label', 'has_variants', 'base_stock'];
                    ...
                    const orderedKeys = Object.keys(baseMeta).filter(k => !skip.includes(k) && !k.toLowerCase().includes('image'));
                    // Add any extra keys from variant
                    if (attrs) {
                        Object.keys(attrs).forEach(k => { if (!orderedKeys.includes(k) && !skip.includes(k) && !k.toLowerCase().includes('image')) orderedKeys.push(k); });
                    }

                    orderedKeys.forEach(k => {
                        // Use variant value if available, otherwise fall back to base product value
                        const val = (attrs && attrs[k]) || baseMeta[k] || '';
                        // Skip any value that is an image URL, array, object, or contains URL patterns
                        const valStr = String(val);
                        if (val && !Array.isArray(val) && typeof val !== 'object' && !k.toLowerCase().includes('image') && !valStr.includes('://') && !valStr.includes('supabase') && !valStr.startsWith('[') && !valStr.startsWith('data:')) {
                            const lbl = k.charAt(0).toUpperCase() + k.slice(1).replace(/_/g, ' ');
                            specsBox.innerHTML += '<div class="spec-chip"><i class="fas fa-check-circle"></i><span><strong>' + lbl + ':</strong> ' + val + '</span></div>';
                        }
                    });
