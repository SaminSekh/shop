...
class SuperAdminManager {
    ...
    async loadUsers() {
        ...
        try {
            const { data: users } = await supabaseClient
                ...
                .select('*, shops:shop_id(id, shop_name)')
                .order('created_at', { ascending: false })
                .limit(5); // Limit to 5 for recent users, but load all for management
        ...
        try {
            ...
            form.innerHTML = `
                ...
                <div class="form-group">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; background: #f1f5f9; padding: 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
                    ...
                    <small class="text-muted" style="display: block; margin-top: 8px; font-style: italic;">
                        Freeze blocks access, Stop hides data from stats, and Suspend shows warning.
                    </small>
                </div>
    ...
    renderStatusBadges(status) {
        ...
        if (status.includes('suspended')) badges.push('<span class="badge badge-warning"><i class="fas fa-exclamation-triangle"></i> Suspend</span>');
