...
class SuperAdminManager {
    ...
    async loadDashboardData(filter = 'all') {
        ...
        try {
            ...
            // Build queries
            let shopsQuery = supabaseClient.from('shops').select('*', { count: 'exact', head: true }).neq('is_paused', true);
            let usersQuery = supabaseClient.from('profiles').select('*', { count: 'exact', head: true });

            // Get NOT paused shop IDs for filtering calculations
            const { data: activeShopsData } = await supabaseClient.from('shops').select('id').neq('is_paused', true);
            const activeShopIds = activeShopsData?.map(s => s.id) || [];

            // For Total Products, we want the count of unique products and total units
            let productsQuery = supabaseClient.from('products').select('id, stock, cost_price, selling_price').in('shop_id', activeShopIds);

            let salesQuery = supabaseClient.from('sales').select('id, total_amount').in('shop_id', activeShopIds);

            // Apply date filter if not 'all'
            if (filter !== 'all' && dateRange.start) {
                // shopsQuery/usersQuery might still count all or filtered by date?
                // The original code was filtering them by date. Let's keep that.
            ...
            console.log('DEBUG: Active Shop IDs:', activeShopIds);

            // Execute queries
            const { count: shopsCount } = await shopsQuery;
            ...
            // Fetch Data for Aggregated Stats (only for active shops)
            const { data: allShops } = await supabaseClient.from('shops').select('id, current_balance').in('id', activeShopIds);

            // Pending Balance (Total Credit Outstanding)
            const { data: allCredits } = await supabaseClient.from('credits').select('pending_amount').in('shop_id', activeShopIds);
            ...
            if (document.getElementById('totalCredit')) document.getElementById('totalCredit').textContent = formatCurrency(totalPendingBalance); // Assuming 'Total Credit' here means total outstanding given the label text in request seems to align with pending

            // Total Expenses (For filter period) - re-fetch with type
            let expenseQuery = supabaseClient.from('expenses').select('amount, expense_type, expense_date').in('shop_id', activeShopIds);
            ...
            if (allShops) {
                ...
                // We need 'allSales' (filtered) -> we have 'sales' variable which is filtered sales.
                // We need 'periodExpenses' (filtered) -> we have it.
                // We need 'allCredits' -> we have 'allCredits' (Snapshot of pending). Do we need filtered credits?
                // Just use snapshot for pending.

                allShops.forEach(shop => {
                    const shopSales = getSum(sales, shop.id, 'total_amount');
                    const shopPending = getSum(allCredits, shop.id, 'pending_amount');
    ...
    renderRecentShops(shops) {
        ...
        tableBody.innerHTML = shops.map(shop => `
            <tr>
                ...
                <td>
                    <span class="badge ${shop.is_paused ? 'badge-danger' : 'badge-success'}">
                        ${shop.is_paused ? 'Paused' : 'Active'}
                    </span>
                </td>
                <td>${formatDate(shop.created_at)}</td>
                <td>
                    <button class="btn btn-sm ${shop.is_paused ? 'btn-success' : 'btn-warning'} pause-shop-btn" data-id="${shop.id}" title="${shop.is_paused ? 'Resume Shop' : 'Pause Shop'}">
                        <i class="fas ${shop.is_paused ? 'fa-play' : 'fa-pause'}"></i>
                    </button>
        ...
        tableBody.querySelectorAll('.pause-shop-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                const shopId = e.currentTarget.dataset.id;
                this.togglePauseShop(shopId);
            });
        });
    ...
    attachShopEventListeners() {
        ...
        tableBody.querySelectorAll('.delete-shop-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                const shopId = e.currentTarget.dataset.id;
        ...
        tableBody.querySelectorAll('.visit-shop-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                const shopId = e.currentTarget.dataset.id;
    ...
    async togglePauseShop(shopId) {
        if (this.isToggling) return;

        const shop = this.shops.find(s => s.id === shopId) || await this.fetchShopById(shopId);
        if (!shop) {
            showNotification('Shop not found', 'error');
            return;
        }

        const newState = !shop.is_paused;
        const action = newState ? 'pause' : 'resume';

        if (!confirm(`Are you sure you want to ${action} this shop? ${newState ? 'Calculations for this shop will be excluded from the dashboard.' : ''}`)) {
            return;
        }
        ...
        try {
            ...
            showNotification(`Shop ${action}d successfully`);

            // Refresh all data to ensure consistency across all tables
            await this.refreshAllData();

        } catch (error) {
            console.error(`Error ${action}ing shop:`, error);
            showNotification(`Failed to ${action} shop: ${error.message || 'Unknown error'}`, 'error');
        } finally {
    ...
    attachUserEventListeners() {
        ...
        tableBody.querySelectorAll('.toggle-user-status').forEach(button => {
            button.addEventListener('click', (e) => {
                const userId = e.currentTarget.dataset.id;
    ...
    async showEditShopModal(shopId) {
        ...
        try {
            ...
            form.innerHTML = `
                ...
                <div class="form-group" style="margin-top: 15px; padding: 10px; background: #fff5f5; border: 1px solid #feb2b2; border-radius: 6px;">
                    <div class="form-check">
                        <input type="checkbox" id="editShopPaused" ${shop.is_paused ? 'checked' : ''}>
                        <label for="editShopPaused" style="color: var(--danger); font-weight: bold;">
                            <i class="fas fa-user-slash"></i> Pause Shop (Block All Logins)
                        </label>
                        <p style="margin: 5px 0 0 25px; font-size: 0.85em; color: #742a2a;">
                            When checked, staff and admins of this shop will be prevented from logging in.
                        </p>
                    </div>
                </div>
            `;
    ...
    async updateShop() {
        ...
        const isPaused = document.getElementById('editShopPaused').checked;
    ...
    formatLogDetails(log) {
        ...
        if (table === 'sales' && action === 'sell') {
            return `Sale: ${data.invoice_number} | Amount: ₹${data.total_amount} | Items: ${data.items_count}`;
        }
        ...
        if (table === 'credits') {
            if (action === 'create') return `New Credit: ${data.buyer_name} | Amount: ₹${data.total_amount}`;
            if (action === 'payment') return `Payment from ${data.buyer_name}: ₹${data.amount}`;
            if (action === 'delete') return `Deleted credit record ID: ${data.credit_id || log.record_id}`;
        }

        if (table === 'expenses') {
            if (action === 'create') return `Added expense: ${data.description} | Amount: ₹${data.amount}`;
